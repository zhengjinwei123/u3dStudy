...
namespace GameServer.DAO
{
	class ResultDAO
	{
		public Result GetResultByUserrid(MySqlConnection conn, int userId) {
			...
			try
			{
				...
				Console.WriteLine("在 GetResultByUserrid 的时候出现异常:" + e);
			} finally {
		...
		public void UpdateOrAddResult(MySqlConnection conn, Result res) {
			try
			{
				...
				cmd.ExecuteNonQuery();
				if (res.Id <= -1) {
					Result tmpRes = GetResultByUserrid(conn, res.UserId);
					res.Id = tmpRes.Id;
				}
			}
			catch (Exception e)
			{
				Console.WriteLine("在UpdateOrAddResult出现异常:" + e);
			}
			finally {
